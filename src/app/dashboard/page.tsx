  ...
  return (
    <div className="space-y-6">
      <Header
        ...
        title="Dashboard"
        description="Bienvenido a Austro POS. Este panel inicial resume el estado base del negocio mientras construimos los módulos operativos."
        action={
          <Button
            variant="secondary"
        ...
        <Card className="border border-slate-200/80 bg-white/95 shadow-sm shadow-slate-200/50 dark:border-white/8 dark:bg-[color-mix(in_oklch,var(--card)_90%,#0a1019)] dark:shadow-[0_10px_24px_-18px_rgba(0,0,0,0.58)]">
          ...
          <CardContent>
            <p className="text-sm leading-6 text-slate-600 dark:text-slate-300">
              La autenticación, protección de rutas y dashboard base ya están
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {dashboardCards.map((card, index) => (
          <Card
            key={card.title}
            className={`border bg-white/95 shadow-sm dark:bg-slate-900/72 dark:shadow-[0_10px_22px_-18px_rgba(0,0,0,0.62)] ${
              index === 2
                ? "border-amber-200/80 shadow-amber-100/45 dark:border-amber-500/18"
                : "border-primary/12 shadow-[0_10px_24px_-18px_var(--brand-shadow-soft)] dark:border-primary/12"
            }`}
          >
            <CardHeader className="space-y-1">
              <CardDescription className="text-sm text-slate-600 dark:text-slate-300">
              ...
              <CardTitle
                className={`text-3xl font-semibold tracking-tight dark:text-white ${
                  index === 2 ? "text-amber-700 dark:text-amber-300" : "text-slate-950"
                }`}
              >
                {card.value}
              </CardTitle>
